...
<script> 
	$(document).ready(function() { 
				$.ajax({
					url : "/rest/list", // 요청 url을 의미한다.
					dataType : "json", // 서버에서 받아올 데이터를 어떤 형태로 해석할 것인지. xml, json, html, script를 선택할 수 있다.
					success : function(data) { // Ajax 통신에 성공했을 때 실행되는 이벤트. 받아온 데이터 넣는다
						console.log(data);

						/* foreach 돌리기 */
						$.each(data, function(key, value) {
							console.log('key: ' + key + ', name: ' + value.ename + ', sal: ' + value.sal);
										
							// 연습용 태그 만들기
							// var tag = "<div>아하 그렇구나</div>";
							
							var random = Math.floor(Math.random()*6)+1; // 랜덤숫자 만들기
							
							var tag ="";
							tag += "<div class='col-sm-4'>";
							tag += "<div class='product-image-wrapper'>";
							tag += "<div class='single-products'>";
							tag += "<div class='productinfo text-center'>";
							tag += "<img src='images/home/product"+random+".jpg' alt='' />";
							// 중간에 변수를 집어넣으려고 '' 중간끊고 문자를 "로 닫아버린 후에 연결함!
							tag += "<h2>"+value.sal+"</h2>";
							tag += "<p>"+value.ename+"</p>";
							tag += "<a href='#' class='btn btn-default add-to-cart'><i class='fa fa-shopping-cart'></i>Add to cart</a>";
							tag += "</div>";
							tag += "<div class='product-overlay'>";
						...
						/* 내가 한거: 완전 잘못함
						$("#sportswear").$("p").text(data.ename); ///만들기
						console.log("ok"); */
					},
					error : function(request, status, error) {
						// ajax 통신에 실패했을 때 실행되는 이벤트. request, staus, error로 에러 정보를 확인할 수 있다.
						console.log(request.responseText);
						console.log(error);
					} /* 고정값 */
				});
				
		/* 		$.ajax({ // <li><a href="#">Nike </a></li>
					url : "./dept.jsp", // 요청 url을 의미한다.
					dataType : "json", // 서버에서 받아올 데이터를 어떤 형태로 해석할 것인지. xml, json, html, script를 선택할 수 있다.
					success : function(data) { // Ajax 통신에 성공했을 때 실행되는 이벤트. 받아온 데이터 넣는다
						console.log(data);
						...
						$.each(data, function(key, values) {
							console.log('key: ' + key + ', dname: ' + values.dnames[0]);
																
							var tag = "<li><a href='#'>"+ num +"</a></li>";
							num += 1;							
							
							$("#sportswear ul").append(tag);
						});
						/* 내가 한거: 완전 잘못함
						$("#sportswear").$("p").text(data.ename); ///만들기
						console.log("ok"); */
		...
		<div class="container">
			<div class="row">
				...
				<div class="col-sm-9 padding-right">
					<div class="features_items" id="features_add">
						<!--features_items-->
